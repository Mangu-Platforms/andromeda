...
5. **Budget enforcement is server-side and resume-safe** (`metering.ts`: ceiling seeded from prior spend on resume; `MeteredProvider` wraps the provider itself), so no client-side bypass exists. Bounded overshoot of one completion is documented. Residual risk: with no auth (B2), any visitor can start N runs — a per-run ceiling but **no global/daily ceiling**. Propose a global meter.
