...
import type { ExecOptions, ExecResult, Sandbox } from "./types.ts";
...
export interface LocalSandboxOptions {
   ...
   * parent process must not be reachable from generated code.
   */
  env?: Record<string, string>;
}

/**
 * Filesystem-and-child-process sandbox.
 *
 * The containment here is real but modest, and worth stating plainly: paths are
 ...
 * files. That is acceptable for running a test suite the pipeline just wrote
 * during development, and not acceptable for untrusted multi-tenant work —
 * bind `Sandbox` to a container for that.
 */
export class LocalSandbox implements Sandbox {
  ...
  private constructor(root: string, options: LocalSandboxOptions) {
    ...
    this.#env = {
      ...
      HOME: root,
      TMPDIR: root,
      // Generated tests must not inherit the parent's colour/CI quirks.
      NO_COLOR: "1",
      ...options.env,
    };
  }
  ...
  static async create(options: LocalSandboxOptions = {}): Promise<LocalSandbox> {
    const root = await mkdtemp(join(tmpdir(), "andromeda-build-"));
  ...
  async exec(command: string, args: string[], options: ExecOptions = {}): Promise<ExecResult> {
    ...
    return new Promise<ExecResult>((resolvePromise, rejectPromise) => {
      const child = spawn(command, args, {
        cwd,
        env: this.#env,
        // No shell: arguments are passed as a vector, so nothing in a
        // generated filename or feature id can be interpreted as a command.
        shell: false,
